...
public class CookieUtil
{
	...
	public static String getCookieValue(HttpServletRequest request, String cookieName)
	{
		...
		for (int i = 0; i < aryCookie.length; i++)
		{
			...
			{
				log.info("getCookieValue 获取COOKIE   [cookieName:" + cookieName + ",cookieValue:" + cookie.getValue() + "]");
			...
			String cookieName, String cookieValue, int age, String path)throws Exception
	{
		...
		cookieName=StrUtils.solutionGarbled(cookieName,"UTF-8");
		cookieValue=StrUtils.solutionGarbled(cookieValue,"UTF-8");
		path=StrUtils.solutionGarbled(path,"UTF-8");
		log.info("设置cookie的入参是编码之后 ： cookieName="+cookieName+"  cookieValue="+cookieValue+" age="+age+"   path="+path);
		Cookie cookie = new Cookie(cookieName, cookieValue);
